...
interface Props {
  setEditDialogOpen: React.SetStateAction<any>;
  food: FoodEntry;
}
const EditFood: React.FC<Props> = (props) => {
  const router = useRouter();
  const { setEditDialogOpen, food } = props;

  const [name, setName] = React.useState<string>(food.name);
  const [details, setDetails] = React.useState<FoodEntryDetails>(food?.details ?? {});
  ...
  return (
    <UpdateFoodWindow>
      ...
      <WindowContent>
        ...
        <br />
        <Button onClick={handleEditFood}>Update Food</Button>
      </WindowContent>
      {info && (
...
const UpdateFoodWindow = styled(Window)`
  position: fixed;
  width: 50%;
  top: 50%;
  left: 50%;
